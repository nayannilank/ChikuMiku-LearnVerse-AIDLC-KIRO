 ...
 * from offline to online on React Native.
 *
...
import { mobileOfflineStore } from '../services/offlineStorage';
...
export function useSyncOnReconnect(serverUrl: string): SyncState {
  ...
  const performSync = useCallback(async () => {
    ...
    try {
      const result = await mobileOfflineStore.syncProgress(serverUrl);
